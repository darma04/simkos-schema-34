            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    requestAnimationFrame(function() {
                        ...
                        const submitBtn = document.querySelector('#editRoleForm button[type="submit"]');
                        const roleNameInput = document.getElementById('editRoleName');
                        
                        
                        const existingAlert = document.getElementById('superuserInfoAlert');
                        if (existingAlert) existingAlert.remove();
                        
                        if (roleCode === 'SUPERUSER') {
                            
                            container.insertAdjacentHTML('afterbegin', '<div id="superuserInfoAlert" class="alert alert-info d-flex align-items-center mb-4"><i class="ri-information-line ri-22px me-2 text-info"></i><div><strong>Akses Penuh:</strong> Role SUPERUSER bypass semua permission checks dan tidak dapat diedit.</div></div>');
                            
                            
                            if (submitBtn) submitBtn.disabled = true;
                            if (roleNameInput) roleNameInput.disabled = true;
                            document.querySelectorAll('#editRoleForm input[type="checkbox"]').forEach(cb => cb.disabled = true);
                        } else {
                            
                            if (submitBtn) submitBtn.disabled = false;
                            if (roleNameInput) roleNameInput.disabled = false;
                            document.querySelectorAll('#editRoleForm input[type="checkbox"]').forEach(cb => cb.disabled = false);
                        }
                    });
                } else {
